    ...
    def parse(cls, src: str) -> Tuple[str, 'PrototypeMap']:
        ...
        while idx < len(buf):
            ...
            if not func:
                ...
                raise SyntaxError(
                    'badly formatted return argument (please use parenthesis and proper arguments naming): ' + func)
...
class Assembler:
    ...
    def _remove_comments(line: str, *, st: str = 'normal') -> str:
        for i, ch in enumerate(line):
            if st == 'normal' and ch == '/':
                st = 'slcomm'
            elif st == 'normal' and ch == '\"':
                st = 'string'
            elif st == 'normal' and ch in ('#', ';'):
                return line[:i]
            elif st == 'slcomm' and ch == '/':
                return line[:i - 1]
            elif st == 'slcomm':
                st = 'normal'
            elif st == 'string' and ch == '\"':
                st = 'normal'
            elif st == 'string' and ch == '\\':
                st = 'escape'
            elif st == 'escape':
