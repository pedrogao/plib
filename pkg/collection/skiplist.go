...
/**
reference: https://ketansingh.me/posts/lets-talk-skiplist/

Hi,
Very interesting topic.
I just discovered go:notinheap pragma to tag a type as not garbage-collected. It could be interesting in such cases.
Max.
*/

type (
	Record[K constraints.Ordered, V any] struct {
		Key   K
		Value V
	}

	SkipNode[K constraints.Ordered, V any] struct {
		record  *Record[K, V]
		forward []*SkipNode[K, V]
	}

	SkipList[K constraints.Ordered, V any] struct {
		head  *SkipNode[K, V]
		level int
		size  int
	}
...
func NewRecord[K constraints.Ordered, V any](key K,
	value V) *Record[K, V] {
	return &Record[K, V]{
		Key:   key,
		Value: value,
...
func NewSkipNode[K constraints.Ordered, V any](key K,
	value V, level int) *SkipNode[K, V] {
	return &SkipNode[K, V]{
